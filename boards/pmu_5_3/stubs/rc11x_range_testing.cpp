 ...
 *
 * The user may toggle between the different states by sending the character 'n'
 * (which stands for "next") over the IR interface.
 *
 * NOTE: This is very similar to the stub associated with Node v1.1 with the same name.
 * The only difference as of this documentation is that the button functionality has
 * been removed from this version, since the PMU does not have a general-purpose button.
 *
 * @section LICENSE
